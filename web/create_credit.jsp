...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-3">
            <h2 class="text-left">Заключение кредитного договора</h2>

            <form class="form-horizontal" action="/controller" method="post">
                <input type="hidden" name="command" value="createCreditForClient">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="creditId" class="form-control" id="sel1">
                            <c:forEach var="credit" items="${credits}">
                                <c:choose>
                                    <c:when test="${credit.type.name eq 'MONTHLY'}">
                                        <option value="${credit.id}">${credit.currency} | Ежемесячный
                                            | ${credit.percent}%
                                        </option>
                                    </c:when>
                                    <c:otherwise>
                                        <option value="${credit.id}">${credit.currency} | Аннуитетный
                                            | ${credit.percent}%
                                        </option>
                                    </c:otherwise>
                                </c:choose>
                ...
                <div class="form-group">
                    <label class="control-label col-sm-2" for="sum">Сумма кредита:</label>
                    <div class="col-sm-6">
                        <input type="number" min="0" step="1" required class="form-control"
                               name="sum" id="sum"
                               maxlength="7"
                               placeholder="Введите сумму кредита в соответствии с валютой депозита">
                    </div>
                </div>
